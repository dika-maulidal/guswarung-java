...
public class MenuDAO {
    ...
    // Ambil satu data menu berdasarkan ID (untuk Edit)
...
public boolean insertMenu(Menu m) {
    String sql = "INSERT INTO menus (nama, deskripsi, harga, stok, kategori, gambar, diskon_persen) VALUES (?, ?, ?, ?, ?, ?, ?)";
    try (Connection con = Koneksi.configDB(); PreparedStatement ps = con.prepareStatement(sql)) {
...
public boolean updateMenu(Menu m) {
    String sql = "UPDATE menus SET nama=?, deskripsi=?, harga=?, stok=?, kategori=?, gambar=?, diskon_persen=? WHERE id=?";
    try (Connection con = Koneksi.configDB(); PreparedStatement ps = con.prepareStatement(sql)) {
        ps.setString(1, m.getNama());
        ps.setString(2, m.getDeskripsi());
        ps.setDouble(3, m.getHarga());
